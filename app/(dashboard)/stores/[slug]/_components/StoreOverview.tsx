...
import type { MaintenanceEvent } from "@/features/maintenance/types";
...
export function StoreOverview({ detail, events }: StoreOverviewProps) {
  const { ticketHistory, reports, equipment } = detail;

  // ── Manutenções ──────────────────────────────────────────────
  const preventiva  = events.filter((e) => e.type === "preventiva").length;
  const corretiva   = events.filter((e) => e.type === "corretiva").length;
  const instalacao  = events.filter((e) => e.type === "instalação").length;
  const totalManut  = events.length;
